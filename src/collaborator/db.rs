...
pub async fn run_migrations(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_votes_comment ON comment_votes(comment_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_votes_user ON comment_votes(user_id)")
        .execute(pool)
        .await?;
...
pub async fn load_and_parse_all_comments(
    ...
    let comments = sqlx::query_as::<_, Comment>("SELECT * FROM comments WHERE status != 'hidden'")
        .fetch_all(pool)
        .await?;
    ...
    for comment in comments {
        if let Some(audit_data) = data_context.get_audit(&comment.audit_id) {
            let mentions = parser::parse_comment(&comment.content_markdown, audit_data);
            let html = formatter::render_comment_html(&comment.content_markdown, &mentions);
...
pub async fn create_comment(
    ...
    let result = sqlx::query(
        r#"
        INSERT INTO comments (audit_id, topic_id, content_markdown, author_id, comment_type, status)
        VALUES (?, ?, ?, ?, ?, ?)
        "#,
    )
    .bind(audit_id)
...
pub async fn get_comment_raw(pool: &SqlitePool, comment_id: i64) -> Result<Comment, sqlx::Error> {
